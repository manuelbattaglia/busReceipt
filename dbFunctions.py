...
IMG_KEY = {'date':'0000-00-00', 'time':'00:00:00', 'total':0.0, 'city':'city', 'address':'address', 'merchant_name':'name', 'number': '0000-0000'}REGISTER_KEY = ['nome', 'cognome', 'password', 'email', 'confirmPassword']DAYS = 10def connectDB():
    '''connect to db'''
        global cursor, connect.   try:
                                        ...
                                        '''generate and return toker from id'''
                                            base = str(id) + str(time.time()/id)
                                                hasher = hashlib.sha256()
                                                    ...
                                                    def deleteExpiredSession()-> None:
                                                        ''' delete expired session'''
                                                      global cursor, conn.   cursor.execute("DELETE FROM session WHERE expire < NOW()")
                                                          conn.commt()
                                                                  ...
                                                                  cursor.execute("SELECT userId FROM session WHERE token = %s", (token,))
                                                                      id = int(cursor.fetchall()[0]['userId'])
                                                                          if(cursor.rowcount == 0):
                                                                                 return -1
                                                                                     return id
                                                                                     def getIdEmail(email: str, password: str)-> int:
